...
def FORP(r):
    ...
    def baseAlgo(r, height=0):
        ...
        boxIn = [{'w': maxHeightRect['w'], 'h':maxHeightRect['h'],
                  'p':maxHeightRect['p'], 'x':0, 'y':0}]
        xWrap = []
        xStep = 1
        ...
        def checkTempBoxInRectLine(rect, x, tempBoxInRectLine):
            ...
            def tempBoxInRectLineSort(elm):
                return elm[0]
            tempBoxInRectLine.sort(key=tempBoxInRectLineSort)
        ...
        for i in range(1, length):
            ...
            for x in range(0, MAXW, xStep):
                ...
                if x == box['w']:
                    boxIn.append({'w': temp_w, 'h': temp_h,
                                  'p': temp_p, 'x': x, 'y': 0})
                    box['w'] += temp_w
                    break
